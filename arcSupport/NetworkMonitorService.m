...
static void NetworkMonitorServiceCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void* info) {
    ...
    NSCAssert([(__bridge NSObject*)info isKindOfClass:[NetworkMonitorService class]], @"info was wrong class in ReachabilityCallback");
   
    // Notify the client that the network reachability changed.
    [(__bridge NetworkMonitorService*)info sendStatusChangedNotification];
}
...
+(NetworkMonitorService*)sharedService {
    ...
    if (!sharedService) {
        sharedService = [self networkMonitorServiceForInternetConnection];
    }
